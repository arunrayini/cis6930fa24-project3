        ...
        for page in reader.pages:
            ...
            if text:
                ...
                for line in lines[3:]:  # Skip the header rows
                    ...
                    if len(fields) == 5:  # Ensure valid data
...
def process_pdf(file_path):
    """
    Process a PDF file to extract incident data using regex for structured data extraction.
    ...
    try:
        ...
        for page_number, page in enumerate(pdf.pages, start=1):
            ...
            print(f"Page {page_number} Text:\n{raw_text}\n{'=' * 50}\n")  # Debugging: Show raw page text

            lines = raw_text.split("\n")
            for line in lines:
                # Regex to match the structure of the incidents
                ...
                if match:
                    ...
                    print(f"Skipping unmatched line: {line}")  # Debugging: Log unmatched lines
    except Exception as e:
...
def create_visualizations(incidents):
    ...
    top_natures = df['nature'].value_counts().head(15).index  # Top 15 most common "nature" values
    df['simplified_nature'] = df['nature'].apply(lambda x: x if x in top_natures else 'Other')

    # Clustering scatter plot
    plt.figure(figsize=(12, 8))
    sns.scatterplot(
        y=df['simplified_nature'],  # Use simplified nature for y-axis
        x=range(len(df)),
    ...
    if df['hour'].dropna().empty:
        ...
        plt.xticks(range(24))  # Ensure all 24 hours are labeled
        plt.tight_layout()
...
def upload_files_and_urls():
    ...
    for file in files:
        if file and file.filename:
            ...
            file.save(file_path)
            incidents = process_pdf(file_path)  # Use process_pdf() here
